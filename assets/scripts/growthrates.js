...
expGraph.width = Math.min(window.innerWidth*.9, 610);
...
logisticGraph.width = Math.min(window.innerWidth*.9, 610);
logisticGraphdNdt.width = logisticGraph.width/2-20;
logisticGraphdNdtN.width = logisticGraph.width/2-20;
...
function updateExpGraph(){
    ...
    let func = "N0 * e ^ ( r * x )";
    graph(func.replaceAll('r', expRSlider.value).replaceAll('N0', expN0Slider.value), expGraph, .005, "Time", "Population", [0, 7], [0, 100], "#00FFA0");
}
function updateLogisticGraph(){
    ...
    graphList(nVals, logisticGraph, .005, "Time", "Population Size", xlim, [0, 1000], "#009FFF");
    graphList(dNdtVals, logisticGraphdNdt, .005, "Population Size", "Population Growth Rate", nlim, [Math.min.apply(null, dNdtVals), Math.max.apply(null, dNdtVals)], "#FF9FFF");
    graphList(dNdtNVals, logisticGraphdNdtN, .005, "Population Size", "Population Growth Rate Per Capita", nlim, [Math.min.apply(null, dNdtNVals), Math.max.apply(null, dNdtNVals)], "#55E0F0");
