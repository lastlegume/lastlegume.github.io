...
function frame(t) {
    ...
    if (runSummarySidebar[0] && highlightIdx[0] >= 0 && highlightIdx[0] < results[0].length)
        drawProbabilityText(oneCopyHist, results[0][highlightIdx[0]], prevReps[0], highlightIdx[0], "normal");
    if (runSummarySidebar[1] && highlightIdx[1] >= 0 && highlightIdx[1] < results[1].length)
        drawProbabilityText(simHist, results[1][highlightIdx[1]], prevReps[1], highlightIdx[1], prevSettings[3]);
...
function drawProbabilityText(canvas, trials, total, number, type) {
    //if type is normal, say prized, else, say in hand
    ...
    ctx.fillText((type=="normal")?`prized`:"in hand", canvas.width - 220, 200, 230);
